...
from sage.structure.element import Element, Matrix, ModuleElement
...
from .utils import (
    ...
    coefficient_dict_to_matrix,
    complex_tuple_from_json,
    complex_tuple_to_json,
    dual_ideal,
    ideal_generator,
    length_from_M,
    list_of_lists_to_limits,
)
...
class HilbertMaassCoefficients(ModuleElement):
    """
    ...
    def __init__(
        ...
        **kwargs: P.kwargs,
        ...
        - `` Q`` - tuple of positive integers. >= M+2
        ...
        EXAMPLES::
            ...
            sage: from maass_forms_hilbert.all import HilbertMaassFormSpace, HilbertMaassCoefficients # doctest: +ELLIPSIS
            ...
        ...
        if isinstance(coefficients, dict):
            coefficients = coefficient_dict_to_matrix(coefficients)
        parent = coefficients.parent()
    ...
    def coordinate_ideals(self):
        r"""
        Return the coordinate ideals for the coefficients.

        OUTPUT:

        - tuple of NumberFieldFractionalIdeal
        ...
        EXAMPLES::
            ...
            sage: C.coordinate_ideals()
            [Fractional ideal (-1/4*a)]
        """
        return self._coordinate_ideals

    def coordinate_ideal_generators(self):
        r"""
        Return the generators of the coordinate ideals.

        OUTPUT:

        - list of generators for the coordinate ideals
        ...
        EXAMPLES::
            ...
            sage: Cmat = matrix(RR, [[1],[2],[3],[4],[5],[6],[7],[8],[9]])
            sage: C = HilbertMaassCoefficients(Cmat, ((-1,1),(-1,1)), spectral_parameter, H)
            sage: C.coordinate_ideal_generators()
            [1/4*a + 1/2]
        """
    ...
    def M(self):
        r"""
        Return the M parameter (bounds for indices).

        OUTPUT:

        - tuple of integer bounds
    ...
    def space(self):
        r"""
        Return the Hilbert Maass form space associated to these coefficients.

        OUTPUT:

        - HilbertMaassFormSpace
        ...
        EXAMPLES::
            ...
            sage: C = HilbertMaassCoefficients(Cmat, ((-1,1),(-1,1)), spectral_parameter, H)
            sage: C.space()
            HilbertMaassFormSpace(Hilbert Modular Group PSL(2) over Maximal Order generated by a in Number Field in a with defining polynomial x^2 - 2 with a = 1.414213562373095?)
        """
        return self._space
    ...
    def set_coefficients(self):
        r"""
        Return the set coefficients dictionary.

        OUTPUT:

        - dictionary of set coefficients
    ...
    def __eq__(self, other):
        r"""
        Check equality with another HilbertMaassCoefficients object.

        INPUT:

        - ``other`` -- another HilbertMaassCoefficients instance
        ...
        EXAMPLES::
            ...
            sage: spectral_parameter = (CC(0.5,1),CC(0.5,1))
            sage: Cmat = matrix(RR, [[1],[2],[3],[4],[5],[6],[7],[8],[9]])
            sage: C1 = HilbertMaassCoefficients(Cmat, ((-1,1),(-1,1)), spectral_parameter, H)
            sage: C2 = HilbertMaassCoefficients(Cmat, ((-1,1),(-1,1)), spectral_parameter, H)
            sage: C1 == C2
            True
        """
    ...
    def __getitem__(self, key: tuple | Integer_t) -> ComplexNumber:
        ...
        if isinstance(key, tuple) and len(key) == 2 and isinstance(key[1], tuple):
            cusp, v = key
        elif isinstance(key, (int, Integer, tuple)):
            v = key
        ...
        try:
            ...
            raise ValueError(f"Can not get coefficient for {cusp}:{v}") from exc

    def get_coefficient_index(self, v: tuple, cusp: int = 0) -> int:
        """
        Get the index of a coefficient tuple.
        ...
        EXAMPLES::
            ...
            sage: Cmat = matrix(RR, [[1],[2],[3],[4],[5],[6],[7],[8],[9]])
            sage: C = HilbertMaassCoefficients(Cmat, ((-1,1),(-1,1)), spectral_parameter, H)
            sage: C.get_coefficient_index((0,0))
            4
        """
    ...
    def keys(self, as_elements=False) -> list:
        """
        Return the keys (indices) of the coefficients.
        ...
        if self._index_tuples:
            ...
            keys_tuple = [map_int_to_tuple(v, self._M) for v in range(self._coefficients.nrows())]
    ...
    def __iter__(self) -> iter:
        ...
        EXAMPLES::
            ...
            sage: H = HilbertMaassFormSpace(QuadraticField(2), cuspidal=False)
            sage: spectral_parameter = (CC(0.5,1),CC(0.5,1))
            sage: Cmat = matrix(RR, [[1],[2],[3]])
            sage: C = HilbertMaassCoefficients(Cmat, ((-1,1),), spectral_parameter, H)
            sage: len(list(C))
            3
        """
    ...
    def _add_(self, other):
        """
        Add two HilbertMaassCoefficients objects, noting that we only add coefficients
        that exist in both objects and ignore the rest.
        ...
        if not isinstance(other, self.__class__):
            raise ValueError("Addition is only defined for HilbertMaassCoefficients objects")
        if other.parent() != self.parent():
            raise ValueError(
                "Addition is only defined for HilbertMaassCoefficients objects with the same parent"
            )
        new_coefficient_dict = {}
        new_index_tuples = []
        for k in self.keys():
            if k in dict(other):
                new_coefficient_dict[k] = self[k] + other[k]
                new_index_tuples.append(k)
        degree = len(self._M[0])
        index_slices = [[x[i] for x in new_coefficient_dict] for i in range(degree)]
        M_new = list_of_lists_to_limits(index_slices)
        return self.__class__(
            new_coefficient_dict,
            M_new,
            self.spectral_parameter(),
            self.space(),
            coordinate_ideals=self.coordinate_ideals(),
            index_tuples=[new_index_tuples],
        )
    ...
    def _sub_(self, other):
        """
        Subtract two HilbertMaassCoefficients objects.
    ...
    def _lmul_(self, right: Element):
        ...
        """
        try:
            self.parent().base_ring()(right)
        except (ValueError, TypeError) as e:
            raise e
        result = self.__copy__()
        result._coefficients = self._coefficients * right
        return result

    def norms(self, use_abs: bool = False):
        r"""
        ...
        EXAMPLES::
            ...
            sage: Cmat = matrix(RR, [[1],[2],[3],[4],[5],[6],[7],[8],[9]])
            sage: C = HilbertMaassCoefficients(Cmat, ((-1,1),(-1,1)), spectral_parameter, H)
            sage: norms_dict = C.norms()
            sage: isinstance(norms_dict, dict)
            True
        """
        elements_by_norm = {}
