...
cdef int besselk_dp_c(double *kbes,double R,double x,double prec,int pref,bool verbose=False) nogil: #double prec=1e-14,int pref=0):
    r"""
    Modified K-Bessel function in double precision. Chooses the most appropriate algorithm.
    ...
    INPUT:
       ...
       - `R` -- parameter (double)
        ...
        - `pref` -- use prefactor (integer, default 0) 
               ...
               = 0 => computes K_iR(x)
        - ``verbose`` -- boolean (default: False) set to True to print debug output.
...
cpdef lngamma(double x,double R,double prec=1E-16):
    r"""
    Logarithm of the Gamma function for complex argument x + i*R.

    This is a wrapper function for my_lngamma that computes the logarithm
    of the Gamma function using the principal branch of the logarithm.

    INPUT:

    - ``x`` -- double, the real part of the argument
    - ``R`` -- double, the imaginary part of the argument
    - ``prec`` -- double (default: 1E-16), precision parameter

    OUTPUT:

    The value of ln(Gamma(x + i*R)) as a complex number.

    EXAMPLES::

        sage: from maass_forms_hilbert.functions.bessel.kbessel_mpfr import lngamma
        sage: # Test with real argument (R=0)
        sage: result = lngamma(2.0, 0.0)  # ln(Gamma(2)) = ln(1) = 0
        sage: abs(result) < 1e-10
        True
        sage: # Test with complex argument
        sage: result = lngamma(1.0, 1.0)  # ln(Gamma(1+i))
        sage: isinstance(result, complex)
        True
    """
...
cpdef besselk_real_rec(RealNumber r,RealNumber x,double eps=0,int verbose=0):
    r"""
    Modified Bessel function K_r(x) using high-precision arithmetic.

    This function computes the modified Bessel function K_r(x) using
    arbitrary precision arithmetic via SageMath's RealNumber type.
    The parameter r must satisfy |r| <= 0.5.

    INPUT:

    - ``r`` -- RealNumber, the order parameter with |r| <= 0.5
    - ``x`` -- RealNumber, the argument, must be positive
    - ``eps`` -- double (default: 0), convergence tolerance
    - ``verbose`` -- int (default: 0), verbosity level for debugging

    OUTPUT:

    The value of K_r(x) as a RealNumber with the same precision as input.

    EXAMPLES::

        sage: from maass_forms_hilbert.functions.bessel.kbessel_mpfr import besselk_real_rec
        sage: from sage.rings.real_mpfr import RealField
        sage: RF = RealField(100)
        sage: r = RF(0.0)
        sage: x = RF(1.0)
        sage: result = besselk_real_rec(r, x)  # doctest: +ELLIPSIS
        sage: # Should be approximately K_0(1) ≈ 0.4210244382407083
        sage: abs(result - RF(0.4210244382407083)) < RF(1e-10)
        True
        sage: # Test with small r
        sage: r = RF(0.25)
        sage: x = RF(2.0)
        sage: result = besselk_real_rec(r, x)
        sage: result > 0  # K_r(x) is always positive
        True
    """
    if r<0:
        r = -r
...
cpdef besselk_real_rec_dp(double r,double x,double eps=0,int verbose=0):
    ...
    This is an alternative implementation of the modified Bessel function
    K_r(x) using double precision arithmetic. It's similar to the function
    in besselk_dp.pyx but may use different computational methods.
    ...
    EXAMPLES::
        ...
        sage: from maass_forms_hilbert.functions.bessel.kbessel_mpfr import besselk_real_rec_dp
        sage: # Test with r=0, should give K_0(1)
        sage: result = besselk_real_rec_dp(0.0, 1.0)  # doctest: +ELLIPSIS
        sage: abs(result - 0.4210244382407083) < 1e-10
        True
        sage: # Test with small r
        sage: result = besselk_real_rec_dp(0.25, 2.0)  # doctest: +ELLIPSIS
        sage: result > 0  # K_r(x) is always positive for real positive arguments
        True
        sage: # Test error conditions
        sage: besselk_real_rec_dp(0.6, 1.0)  # doctest: +IGNORE_EXCEPTION_DETAIL
        Traceback (most recent call last):
        ...
        ValueError: Use only for r in [-0.5,0.5]
        sage: besselk_real_rec_dp(0.0, 0.0)  # doctest: +IGNORE_EXCEPTION_DETAIL
        Traceback (most recent call last):
        ...
