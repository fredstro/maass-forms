"""Regression tests for the maass_forms_klein.database [db]-extras guard."""
...
def _force_reimport(monkeypatch, missing):
    ...
    monkeypatch.delitem(sys.modules, "maass_forms_klein.database", raising=False)
    monkeypatch.delitem(sys.modules, "maass_forms_klein.database.models", raising=False)
...
def test_guard_fires_when_extras_missing(monkeypatch, missing):
    ...
    with pytest.raises(ModuleNotFoundError, match=r"\[db\] extras"):
        importlib.import_module("maass_forms_klein.database")


@pytest.mark.parametrize("missing", ["mongoengine", "comp_manager"])
def test_guard_message_names_install_command(monkeypatch, missing):
    _force_reimport(monkeypatch, missing)
    with pytest.raises(ModuleNotFoundError, match=r"pip install 'maass_forms_klein\[db\]'"):
        importlib.import_module("maass_forms_klein.database")
