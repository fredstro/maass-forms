...
from knot_maass.utils.json_converters import dict_from_json
...
def search_eigenvalues(space: KleinianMaassFormSpace, R1: Real_t, R2: Real_t,
    ...
    EXAMPLES:
        ...
        sage: from knot_maass.modform.search import search_eigenvalues
    ...
    for r in range_of_r[1:]:
        ...
        h0 = h(RR(r0), space, M, Q, Y1, Y2, set_coefficients=set_coefficients,
               coefficient_indices=coefficient_indices)
        h1 = h(RR(r), space, M, Q, Y1, Y2, set_coefficients=set_coefficients,
               coefficient_indices=coefficient_indices)
...
def newton_method_search(r0: Real_t, r1: Real_t, space: KleinianMaassFormSpace, M: Integer_t,
    ...
    r = newton_method_step(r0, r1, space, M, Q, Y1, Y2, set_coefficients,
                           coefficient_indices=coefficient_indices,
                           tolerance=tolerance,
                           real_imag=real_imag)
    ...
    f = h(r, space, M, Q, Y1, Y2, set_coefficients, coefficient_indices=coefficient_indices,
          real_imag=real_imag)
    log.debug(f"h({r}) = {f}")
    ...
    while err > tolerance:
        try:
            r_new = newton_method_step(r0, r, space, M, Q, Y1, Y2, set_coefficients,
                                       coefficient_indices=coefficient_indices,
                                       tolerance=tolerance,
                                       real_imag=real_imag)
            ...
            r_new = newton_method_step(r, r1, space, M, Q, Y1, Y2, set_coefficients,
                                       coefficient_indices=coefficient_indices,
                                       tolerance=tolerance,
                                       real_imag=real_imag)
        ...
        if abs(r - r_new) < tolerance:
            break
        f = h(r_new, space, M, Q, Y1, Y2, set_coefficients,
              coefficient_indices=coefficient_indices,
              real_imag=real_imag)
        r = r_new
        log.debug(f"f(r_new)={f}")
        err = sum([abs(x) for x in f])
    return r, f
...
def brute_force_search(r0, r1, numpts, space, M, Q, Y1, Y2, set_coefficients: dict = None,
                       coefficient_indices: tuple[Integer_t, ...] = None,
                       tolerance: Real_t = 1e-10, verbose: bool = False):
    ...
    for n, r00 in enumerate(pts):
        ...
        try:
            search = newton_method_search(r00, r01, space, M, Q, Y1, Y2, set_coefficients,
                                      coefficient_indices=coefficient_indices,
                                      tolerance=tolerance, verbose=verbose)
            print("search=", r00, r01, search)
            res.append(search)
