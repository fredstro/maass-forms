...
Note: These are about 250 times faster than the SageMath's standard 'Bessel_K' implementation. 
...
cpdef besselk_dp(double R, double x, double prec=1e-14, int pref=0, algorithm='default'):
    ...
    if algorithm != 'default':
        ...
        if res == 1:
            raise ValueError, 'The K-bessel routine failed (k large) for x,R={0},{1}, value={2}'.format(
                x, RR, kbes)
        elif res == 2:
            raise ValueError, 'The K-bessel routine failed (too many iterations)  for x,R={0},{1}, value={2}'.format(
                x, RR, kbes)
        return kbes
    if x < R * 0.7:
        res = besselk_dp_pow(RR, x, &kbes, prec, pref)
    ...
    if res == 1:
        ...
        raise ValueError, 'The K-bessel routine failed (unknown error)   for x,R={0},{1}, value={2}'.format(
            x, RR, kbes)

cdef int besselk_dp_c(double *kbes, double R, double x, double prec,
                      int pref) nogil:  #double prec=1e-14,int pref=0):
    r"""
    Modified K-Bessel function in double precision. Chooses the most appropriate algorithm.
    ...
    if res == 1:
        ...
        printf('the K-bessel routine failed (too many iterations) for x,R=%g,%g, value=%g', x, RR,
               kbes[0])
    return res

@cython.cdivision(True)
cdef int besselk_dp_rec(double R, double x, double *val, double prec=1e-14,
                        int pref=0) nogil:  # |r| <<1000, x >> 0 !
    r"""
