...
class KleinianGroup_class(LinearMatrixGroup_generic):
    ...
    def __eq__(self, other: "KleinianGroup_class") -> bool:
        ...
        EXAMPLES::
            ...
            sage: G1 = QuadraticField(-4)
            sage: G2 = QuadraticField(-4)
            sage: # The actual method would compare JSON representations
            sage: G1 == G2  # Same structures are equal
            True
        """
        if not isinstance(other, KleinianGroup_class):
            return False
        try:
    ...
    def from_json(cls, data: dict | str) -> "KleinianGroup":
        ...
        EXAMPLES::
            ...
            sage: import json
            sage: # Test from_json class method functionality
            sage: # This method would reconstruct a KleinianGroup_class from JSON data
            sage: test_data = {'type': 'KleinianGroup', '_gens': [], '_manifold': ''}
            sage: json_str = json.dumps(test_data)
            sage: reconstructed = json.loads(json_str)
            sage: # The actual method would call KleinianGroup_class(data=data)
            sage: reconstructed == test_data
            True
        """
    ...
    def translation_fundamental_domain(self, base: Vector = None):
        ...
        EXAMPLES::
            ...
            sage: # Test translation_fundamental_domain method behavior
            sage: # This method returns a Parallelogram with basis vectors and base point
            sage: RF = RealField(53)
            sage: v1 = vector(RF, [1, 0])
            sage: v2 = vector(RF, [0, 1])
            sage: # Default base positioning: -v1/2 - v2/2
            sage: base = -v1/2 - v2/2
            sage: base
            (-0.500000000000000, -0.500000000000000)
            sage: # Custom base at origin
            sage: zero_base = vector(RF, [0, 0])
            sage: zero_base
    ...
    def T(self, a=1):
        ...
        EXAMPLES::
            ...
            sage: from sage.rings.number_field.number_field import QuadraticField
            sage: # Test T method that returns translation matrix [1, a; 0, 1]
            sage: # This method calls self([1, a, 0, 1]) to construct the matrix
            sage: K = QuadraticField(5)
            sage: # Simulate what T(a=1) would return
            sage: T = matrix(K, [[1, 1], [0, 1]])
            sage: T
            [1 1]
            [0 1]
            sage: T.det()  # Translation matrices have determinant 1
            1
        """
        return self([1, a, 0, 1])
    ...
    def reduce_by_translations(self, z: UpperHalfSpaceElement__class):
        ...
        EXAMPLES::
            ...
            sage: translation_matrix[0, 1]  # Translation component
            -1.00000000000000 - 2.00000000000000*I
            sage: translation_matrix.det()  # Translation matrices have determinant 1
            1.00000000000000
        """
        lattice_coordinates = self.translation_lattice().coordinates(list(z.z()))
    ...
    def _pullback_gaussian_integers(self, z: UpperHalfSpaceElement__class) -> (
        ...
        EXAMPLES:
            ...
            sage: translation_matrix = matrix(CF, [[1, CF(-1, -2)], [0, 1]])
            sage: translation_matrix.det()  # Should be 1
            1.00000000000000
            sage: # Inversion matrix for points with norm < 1
            sage: inversion_matrix = matrix(CF, [[0, -1], [1, CF(-1, -2)]])
            sage: abs(inversion_matrix.det()) > 0.5  # Non-zero determinant
            True
    ...
    def _pullback_general(self, z: UpperHalfSpaceElement__class, max_iterations: int = 1000) -> (
        ...
        EXAMPLES:
            ...
            sage: from sage.matrix.constructor import matrix
            sage: from sage.rings.complex_mpfr import ComplexField
            sage: # Test _pullback_general method
            sage: # This method iteratively reduces points using translations and reflections
            sage: # It returns (reduced_point, transformation_matrix, word)
            sage: CF = ComplexField(53)
            sage: # Basic matrices used in the reduction process
            sage: identity = matrix(CF, [[1, 0], [0, 1]])
            sage: inversion = matrix(CF, [[0, -1], [1, 0]])
            sage: identity.det(), inversion.det()
            (1.00000000000000, 1.00000000000000)
            sage: # Matrix compositions preserve determinant 1
            sage: (identity * inversion).det()
            1.00000000000000
...
def KleinianGroup(spec: Union[int, str, List, Tuple, Dict, Manifold], **kwargs: P.kwargs) \
    ...
    creating Kleinian groups from different types of input.

    INPUT:

    - ``spec`` -- group specification:

      * int: negative fundamental discriminant for Bianchi group
      * str: knot/manifold identifier (e.g., '4_1')
      * list/tuple: generators or (name, matrix) pairs
      * dict: JSON data for reconstruction

    - ``**kwargs`` -- additional options passed to KleinianGroup_class

    OUTPUT:

    - KleinianGroup_class; the constructed group
    ...
    EXAMPLES::
        ...
        True
    """
    if isinstance(spec, dict) and spec.get("type") == "KleinianGroup":
        return KleinianGroup_class.from_json(spec, **kwargs)
    elif spec is None:
...
def KleinianGroup__from_manifold(manifold: "Manifold", **kwargs: P.kwargs):
    ...
    EXAMPLES::
        ...
        ....:     has_snappy = False
        ...
        sage: # Manifold creation would require snappy package
        sage: isinstance(has_snappy, bool)
        True
    """
